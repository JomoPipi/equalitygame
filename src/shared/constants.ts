...
export const Colors = ['red','blue','green','purple','orange','gray'] as const

export const QUESTIONS = (() => { return (<Question[]>
  ...
  , { text: 'Are the sums of digits equal?'
    ...
    , generateAB() {
        ...
        const numsA : number[] = []
        const numsB : number[] = []
        for (const [sum, nums] of [[sumA, numsA], [sumB, numsB]] as const)
        {
          for (let i = 0; i < sum;)
          {
  ...
  function chance(percent : number) { return Math.random() < percent }
