...
export function face(canvas, SIZE) {
    ...
    addProperty(self, 'nose', [...Array(5).keys()].map(x => noseImages[x].src));
        ...
        drawHead: {
            ...
            draw(() => {
                for (let i = 0; i < length; i++) {
                    ctx.arc(SIZE * 250 / SCALE, SIZE * (200 + i) / SCALE, SIZE * (100 + i / fat) / SCALE | 0, 0, 7);
                }
            });
        }
        drawMouth: {
            ...
            ctx.fillStyle = 'black';
            draw(() => ctx.ellipse(250 * SIZE / SCALE, height * SIZE / SCALE, width * SIZE / SCALE, stretch * SIZE / SCALE, 0, 0, 7));
            if (smiling) {
                ctx.fillStyle = self.color.arr;
                draw(() => ctx.rect((250 - width) * SIZE / SCALE, (height - 60) * SIZE / SCALE, (width * 2) * SIZE / SCALE, 60 * SIZE / SCALE));
            }
        }
        drawNose: {
            const size = 61 * SIZE / SCALE;
            ctx.drawImage(self.nose.img, SIZE / 2 - size / 2, 0.45 * SIZE, size, size);
        }
        drawEyes: {
            ...
            for (let A of [SIZE / 2 - spread * SIZE / SCALE, SIZE / 2 + spread * SIZE / SCALE])
                for (let [B, C] of [[radiusW, 'white'], [radiusB, 'black']]) {
                    ctx.fillStyle = C;
                    draw(() => ctx.arc(A, height * SIZE / SCALE, B * SIZE / SCALE, SA, EA));
                }
        }
        drawHair: {
            ...
            for (let i = 0; i < I; i++)
                for (let j = 0; j < J; j++) {
                    ctx.fillStyle = 'black';
                    draw(() => ctx.ellipse((xcoord + 1 - x * 5 + j * 20) * SIZE / SCALE, height * SIZE / SCALE, 70 * SIZE / SCALE, (17 + j / 2 | 0) * SIZE / SCALE, i, 0, 7));
                    ctx.fillStyle = 'purple';
                    draw(() => ctx.ellipse((xcoord - x * 5 + j * 20) * SIZE / SCALE, height * SIZE / SCALE, 70 * SIZE / SCALE, (22 - j) * SIZE / SCALE, i, 0, 7));
                }
        }
    };
    return { render() { self.show(); }, canvas, next: { hair: self.hair.next,
            eyes: self.eyes.next,
...
//# sourceMappingURL=getface.js.map